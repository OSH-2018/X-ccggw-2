...
def erasure_encoder(pkg_name, block_index):
    ...
    # block_manager.pack_sent_data(index=block_index)
    block_manager.pack_sent_data(index=block_index, remove=True)
...
def main1():
    """collect package and give back the address"""
...
def main2():
    temp = input("the package\n>>>")
    erasure_decoder(temp)


def main3():
    temp = input("the package\n>>>")
    build_deb(temp)
    shutil.rmtree("{}/donut".format(os.getcwd()))  # package folder
    shutil.rmtree("{}/Coding".format(os.getcwd()))  # block folder


if __name__ == "__main__":
    print("Mode:")
    print("1. get zipped package and erasure encode")
    print("2. erasure decode")
    print("3. build deb package")
    d = input(">>>")
    if d == '1':
        main1()
    elif d == '2':
