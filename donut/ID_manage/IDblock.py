import json
import xmlrpc.client
from xmlrpc.server import SimpleXMLRPCServer
import hashlib
import socket
import os
from client import Download


myname = socket.getfqdn(socket.gethostname())
myip = socket.gethostbyname(myname)
# 默认DNSip，可初始化修改
DNSip = "192.168.43.195"
# 默认软件使用端口
DNSport = "1234"
port = "1234"
# 默认ID表块数
num = 6


class IDServer(object):

    #   常驻的关于被动ID表块更新，创建，管理备份的类

    def __init__(self):
        self.addID()
        self.create_IDblock()
        self.identity_judge()

    # ID表块项的添加
    def addID(self, blockname, ID):
        # 判断表块是否存在，若不存在先创建
        if not os.path.exists('./block_save/%s.txt' % blockname):
            self.create_IDblock(blockname)
            
        # 添加新的表块ID项
        with open('./block_save/%s.txt' % blockname, 'r+') as f:
            block = json.loads(f.read())
            block["IDlist"].append(ID)
            json_block = json.dumps(block)
            f.write(json_block)
            
        # 重新生成验证信息
        verification = hashlib.md5(json_block)
        with open('./block_save/%s.md5' % blockname, 'w') as f:
            f.write(verification)
        return 1                              # 成功更新


    def create_IDblock(self, blockname):
        
        # 若是已经存在则不需要创建
        if os.path.exists("./block_save/" + blockname + '.txt'):
            return 0
        
        # 初始化信息
        block = {}
        IDlist = []
        """
            结构为：{
                "name" : %s
                "IDlist": []
            }
        """
        block["IDlist"] = IDlist
        block["name"] = blockname
        
        # 导为json字符串
        json_block = json.dumps(block)
        
        # 生成验证信息，并保存块与验证信息
        verification = hashlib.md5(json_block)
        with open("./block_save/%s.txt" % blockname, 'w') as f:
            f.write(json_block)
        with open("./block_save/%s.md5" % blockname, 'w') as f:
            f.write(verification)
        return 1                      # 成功创建

    def identity_judge(self):
        
        # 获取自身与自身两侧最近的节点的ID
        ID = get_myID()
        IDl = get_myleftID()
        IDr = get_myrightID()
        
        # 获取自身与自身两侧最近节点所保存的ID表块的信息
        blocknamedir = get_blocknamedir()

        """
            1遍历左节点中保存的ID表块
            2验证身份
                    1）若左节点是管理者，则备份左节点的此表块
                    2）若本节点是管理者且未存此表块（常在本节点刚上线时出现），则下载左节点的此表块
                    3）若其他节点是管理者，则与本节点无关
        """
        for blockname in blocknamedir['leftID']:
            managerID = findID(hashlib.md5(blockname))
            if managerID == IDl or (managerID == ID and not blockname in blocknamedir['myID']):
                DNSc = xmlrpc.client.ServerProxy("http://" + DNSip + ":" + DNSport + "/")
                targetip = DNSc.getip(IDl)
                Download([targetip, targetip], port, [blockname+'.txt', blockname+'.md5'])

        """
            同上遍历右节点的ID表块
        """
        for blockname in blocknamedir['rightID']:
            managerID = findID(hashlib.md5(blockname))
            if managerID == IDr or (managerID == ID and not blockname in blocknamedir['myID']):
                DNSc = xmlrpc.client.ServerProxy("http://" + DNSip + ":" + DNSport + "/")
                targetip = DNSc.getip(IDr)
                Download([targetip, targetip], port, [blockname+'.txt', blockname+'.md5'])

        """
            1遍历本阶段的ID表块
            2判断管理者是否在左本右三个节点之间
                    若不在，则本节点无需存此表块，执行删除表块及其验证信息的操作
        """
        for blockname in blocknamedir['myID']:
            managerID = findID(hashlib.md5(blockname))
            if managerID != ID and managerID != IDl and managerID != IDr:
                blockfile = './block_save/%s.txt' % blockname
                verifyfile = './block_save/%s.md5' % blockname
                if os.path.exists(blockfile):
                    os.remove(blockfile)
                if os.path.exists(verifyfile):
                    os.remove(verifyfile)


class IDop(object):

    # 本类用于存一些外部可见，可用的ID表块操作

    def __init__(self):
        self.addmyID()
        self.collectIDblock()
        self.create_IDblocks()
        self.strcmplmt()
    
    def create_IDblocks(filename):

        # 创建新的表块们（常见于新的软件包的加入），目的是为了保证即使表块为空，也要存在

        # 生成表块名 list
        blocknames = ['%sIDblock_%d' % (filename, x) for x in range(num)]

        # hash得到应存表块的理想ID list
        _targetID = list(map(hashlib.md5, blocknames))

        # 获取实际存在的ID list
        targetID = list(map(findID, _targetID))

        # 翻译成为ip（未进行ip存入十分表的修改）
        DNSc = xmlrpc.client.ServerProxy("http://" + DNSip + ":" + DNSport + "/")
        targetip = list(map(DNSc.getip, targetID))  # -------------------

        # 发送建表请求
        for i in range(num):
            communication = xmlrpc.client.ServerProxy("http://" + targetip[i] + ":" + port + "/")
            communication.create_IDblock(blocknames[i])

    def strcmplmt(x):
        return x + ".txt"

    def collectIDblock(filename):

        # 收集ID表块

        blocknames = ['%sIDblock_%d' % (filename, x) for x in range(num)]
        _targetID = list(map(hashlib.md5, blocknames))
        targetID = list(map(findID, _targetID))
        DNSc = xmlrpc.client.ServerProxy("http://" + DNSip + ":" + DNSport + "/")
        targetip = list(map(DNSc.getip, targetID))  # -------------------
        blockfilenames = list(map(strcmplmt, blocknames))
        Download(targetip, port, blockfilenames)  # 多线程下载 multiDownload(ip[, file[,)
        verifications = ["%sIDblock_%d.md5" % (filename, x) for x in range(num)]
        Download(targetip, port, verifications)
        # 验证文件完整
        errorlist = []
        for i in range(num):
            with open(verifications[i], 'r') as fv:
                with open(blockfilenames[i], 'r') as fb:
                    if hashlib.md5(fb.read()) != fv.read():
                        errorlist.append(i)
        return errorlist  # 返回验证失败的块号

    def addmyID(filename):

        # 通知加入本节点ID入表块的方法

        ID = get_myID()

        # 算出本ID应存入的表块号
        blocknum = hashlib.md5(ID) % num

        # 执行一系列翻译
        _targetID = hashlib.md5("%sIDblock_%d" % (filename, blocknum))
        targetID = findID(_targetID)  # 找到最终的ID-----------------------------------------
        DNSc = xmlrpc.client.ServerProxy("http://" + DNSip + ":" + DNSport + "/")
        targetip = DNSc.getip(targetID)

        # 通知执行更新表块操作
        communication = xmlrpc.client.ServerProxy("http://" + targetip + ":" + port + "/")
        if communication.addID("%sIDblock_%d" % (filename, blocknum), ID) == "addID success":
            return 0
        else:
            print("Fail to connect to target node!")
            ID_lost(ID)
            return -1


if __name__ == "__main__":
    # 开启后台服务程序
    server = IDServer()
    s = SimpleXMLRPCServer(("0.0.0.0", int(port)))
    print("start IDservice...")
    s.register_instance(server)
    s.serve_forever()