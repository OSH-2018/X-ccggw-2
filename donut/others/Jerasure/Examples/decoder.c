...
int main (int argc, char **argv) {
	...
	sprintf(fname, "%s/%s/%s_meta.txt", curdir, dir, cs1);
	...
	while (n <= readins) {
		...
		for (i = 1; i <= k; i++) {
			sprintf(fname, "%s/%s/%s_k%0*d%s", curdir, dir, cs1, md, i, cs2);
			fp = fopen(fname, "rb");
			if (fp == NULL) {
				erased[i-1] = 1;
		...
		for (i = 1; i <= m; i++) {
			sprintf(fname, "%s/%s/%s_m%0*d%s", curdir, dir, cs1, md, i, cs2);
				fp = fopen(fname, "rb");
			if (fp == NULL) {
				erased[k+(i-1)] = 1;
